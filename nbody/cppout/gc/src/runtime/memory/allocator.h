...
private:
    ...
    std::set<PageInfo*> pendinggc_pages; // Pages that are pending GC
    std::set<PageInfo*> filled_pages; // Pages with over 90% utilization (no need for buckets here)
...
public:
    GCAllocator(const GCAllocator&) = delete;              // copy ctor
    GCAllocator(GCAllocator&&) = delete;                   // move ctor
    GCAllocator& operator=(const GCAllocator&) = delete;   // copy assignment
    GCAllocator& operator=(GCAllocator&&) = delete;        // move assignment  

#ifdef MEM_STATS
    GCAllocator(uint16_t objsize, uint16_t fullsize, void (*collect)()) noexcept : freelist(nullptr), evacfreelist(nullptr), alloc_page(nullptr), evac_page(nullptr), allocsize(objsize), realsize(fullsize), pendinggc_pages(), filled_pages(), alloc_count(0), alloc_memory(0), collectfp(collect), inuse() { }
#else 
    GCAllocator(uint16_t objsize, uint16_t fullsize, void (*collect)()) noexcept : freelist(nullptr), evacfreelist(nullptr), alloc_page(nullptr), evac_page(nullptr), allocsize(objsize), realsize(fullsize), pendinggc_pages(), filled_pages(), collectfp(collect), inuse() { }
#endif

    std::list<PageInfo*> inuse;
...
#else 
#endif
    // Page cannot be alloc, evac, or pendinggc
    PageInfo* tryRemovePage(PageInfo* p) {
        if(p == alloc_page || p == evac_page) {
            return nullptr;
        }

        if(this->pendinggc_pages.find(p) != this->pendinggc_pages.end()) {
            return nullptr;
        }
        
        if(this->filled_pages.find(p) != this->filled_pages.end()) {
            this->filled_pages.erase(p);
            return p;
        }

        // This is slow but we dont want to use set for this we need constant access
        auto inuseit = std::find(this->inuse.begin(), this->inuse.end(), p);
        if(inuseit != this->inuse.end()) {
            this->inuse.erase(inuseit);
            return p;
        }
