...
inline void db_bench(std::vector<size_t>& ops) noexcept {
	...
	// TODO made some changes to how our allocations are called, so need
	// to update cases 8 and 9!
	for(size_t& op : ops) {
		switch(op) {
...
// I could not tell you what was going in my brain when I wrote these functions
...
__CoreCpp::Nat main() {
	...
	// Just to make things easier, one of the workloads runs on main thread
	run(mode, gen);
